...
class GeoapifyService
{
    ...
    {
        $query = trim($query);
        if ($query === '' || trim($this->autocompleteApiKey) === '') {
            return [];
        }
    ...
    {
        if (trim($this->autocompleteApiKey) === '') {
            return null;
        }
        ...
        $item = is_array($results[0] ?? null) ? $results[0] : null;
        if ($item === null) {
            return null;
        }
    ...
    private function requestJson(string $url, array $query, int $ttlSeconds, string $prefix, string $apiKey): array
    {
        ...
        $cacheKey = sprintf('geoapify_%s_%s', $prefix, md5($url . ':' . json_encode($query)));
